...
# Manual pick result overrides — use when automatic matching returns wrong results.
# The key is the pick string exactly as it appears in the spreadsheet, lowercased
# and with any location codes like (oh) stripped.  Run process_bracket.py once
# and look at what the site shows to find the normalized key, OR just use the raw
# pick string (normalization is applied automatically).
# Format: {"normalized pick": (wins, is_eliminated, has_played)}
# Examples:
#   "miami / smu": (0, True, True)   — Miami OH/SMU play-in: Miami OH lost, show 0 wins / eliminated
#   "nc state / texas": (1, False, True) — Texas won the play-in, show 1 win / active
PICK_OVERRIDE: dict = {
    "miami / smu": (0, True, True),   # Miami OH lost to Tennessee; play-in win doesn't count
}
...
TEAM_ALIASES = {
    ...
    "cal baptist":  "california baptist",# ESPN: "California Baptist Lancers"
    "penn":         "pennsylvania",      # ESPN: "Pennsylvania Quakers" (Ivy League Penn)
...
def _is_first_four_event(event: dict) -> bool:
    ...
    ESPN doesn't always use a consistent label, so we check multiple fields
    and accept several equivalent terms.
    """
    _FF = ("first four", "first 4", "play-in", "play in", "opening round")

    def _has_ff(s: str) -> bool:
        s = s.lower()
        return any(t in s for t in _FF)

    # Check event-level name / shortName / description
    for field in ("name", "shortName", "description"):
        if _has_ff(str(event.get(field, ""))):
            return True

    # Check season slug / name (e.g. "first-four")
    season = event.get("season", {})
    if isinstance(season, dict):
        for field in ("slug", "displayName", "name"):
            if _has_ff(str(season.get(field, ""))):
                return True
    ...
    # Check competition notes — all text fields in every note object
    for note in comp.get("notes", []):
        for field in ("headline", "type", "text"):
            if _has_ff(str(note.get(field, ""))):
                return True

    # Check competition type descriptor
    comp_type = comp.get("type", {})
    if isinstance(comp_type, dict):
        for field in ("text", "abbreviation", "name"):
            if _has_ff(str(comp_type.get(field, ""))):
                return True

    # ----------------------------------------------------------------
    # Structural fallback — most reliable, requires no ESPN label:
    # First Four games ALWAYS match two teams of the SAME seed
    # (e.g. 11 vs 11, 16 vs 16).  Every Round-of-64 game has two
    # different seeds (1 vs 16, 8 vs 9, …), so this cannot produce
    # a false positive for regular bracket games.
    # ----------------------------------------------------------------
    competitors = comp.get("competitors", [])
    if len(competitors) == 2:
        seeds = []
        for c in competitors:
            s = c.get("seed")  # Only direct tournament seed; curatedRank is AP ranking (99=unranked), not seed
            if s is not None:
                try:
                    seeds.append(int(s))
...
def _normalize(name: str) -> str:
    """
    Lowercase, strip, and canonicalise a team name for matching.

    Special handling:
      • Hawaiian okina / typographic apostrophes → removed
        ("Hawai'i Rainbow Warriors" → "hawaii rainbow warriors")
      • Short state/city codes in parentheses → removed
        ("Miami (OH) RedHawks" → "miami redhawks",
         "Queens (NY) Royals"  → "queens royals")
      • Alias substitution (e.g. "uconn" → "connecticut")
      • Duplicate-entry suffixes like "(2)" → removed
    """
    name = name.lower().strip()
    # Remove Hawaiian okina (U+02BB) and typographic apostrophes/quotes
    name = re.sub(r"[\u02bb\u02bc\u2018\u2019\u02b9ʻ]", "", name)
    # Remove short location/state codes in parens — (ny), (oh), (nc), etc.
    name = re.sub(r"\([a-z]{1,3}\)", "", name)
    name = re.sub(r"\s+", " ", name).strip()
    # Alias substitution (done BEFORE plain-apostrophe strip so keys like
    # "st john's" still match correctly at this stage)
    for alias, canonical in TEAM_ALIASES.items():
        if name == alias:
            name = canonical
            break
    # Strip plain apostrophes AFTER alias lookup.
    # This handles ESPN using a regular ' in "Hawai'i" (pick says "Hawaii"),
    # and also normalises canonical values like "st. john's" → "st. johns"
    # so that both the pick side and ESPN side compare identically.
    name = name.replace("'", "")
    name = re.sub(r"\s+", " ", name).strip()
    # Remove duplicate-submission suffixes like "(2)"
    name = re.sub(r"\(\d+\)", "", name).strip()
    return name


def _loc_code(s: str) -> str:
    """Extract a short location/state code from a raw team string.

    e.g. 'oh' from 'Miami (OH) RedHawks' or 'Miami (oh)'
    Used as a Jaccard tiebreaker when multiple teams share the same city name.
    """
    m = re.search(r"\(([a-z]{1,3})\)", s.lower())
    return m.group(1) if m else ""


def _pick_forms(pick_str: str) -> list[str]:
    """
    Return all normalised candidate forms for a pick string.

    Returns [primary, raw, *reverse_aliases] so that matching works
    bidirectionally.  This is needed because some teams have both an
    abbreviation and a full name (UConn / Connecticut), and ESPN might
    use either.  Trying all forms ensures the pick matches regardless of
    which display name ESPN chooses.

      "UConn"       → ["connecticut", "uconn"]
      "Connecticut" → ["connecticut", "uconn"]   (reverse-alias added)
      "Hawaii"      → ["hawaii"]                  (no alias; char-norm handles Hawai'i)
      "Queens (NY)" → ["queens"]                  (parens stripped)
    """
    primary = _normalize(pick_str)
    forms: list[str] = [primary]

    # Raw form: same char-normalisation steps but WITHOUT alias substitution.
    # Lets "uconn" still be tried even after the alias maps it to "connecticut".
    raw = pick_str.lower().strip()
    raw = re.sub(r"[\u02bb\u02bc\u2018\u2019\u02b9ʻ]", "", raw)
    raw = re.sub(r"\([a-z]{1,3}\)", "", raw)
    raw = re.sub(r"\s+", " ", raw).strip()
    raw = raw.replace("'", "")           # strip plain apostrophes (matches _normalize)
    raw = re.sub(r"\s+", " ", raw).strip()
    raw = re.sub(r"\(\d+\)", "", raw).strip()
    if raw not in forms:
        forms.append(raw)

    # Reverse aliases: any alias key whose canonical value equals primary.
    # e.g. "uconn" → "connecticut": pick="Connecticut" gets primary="connecticut",
    # so we also try "uconn" to handle ESPN using "UConn Huskies".
    for key, val in TEAM_ALIASES.items():
        if val == primary and key not in forms:
            forms.append(key)
...
def match_team_wins(pick: str, wins: dict, eliminated: set) -> tuple[int, bool, bool]:
    ...
    # ── Manual pick override (escape hatch for edge cases) ─────────────────
    # Normalize the pick the same way _normalize() does, then check PICK_OVERRIDE.
    _override_key = _normalize(pick_str)   # strips (oh), lowercases, etc.
    if _override_key in PICK_OVERRIDE:
        return PICK_OVERRIDE[_override_key]
    # ────────────────────────────────────────────────────────────────────────

    if "/" in pick_str:
        # Play-in (First Four) pick: "Team A / Team B"
        # Each part is tried using all its candidate forms (_pick_forms) so
        # that bidirectional alias matching and char-normalisation apply.
        parts = [p.strip() for p in pick_str.split("/")]

        # Step 1: exact match across all forms
        for part in parts:
            for pf in _pick_forms(part):
                for team, w in wins.items():
                    if _normalize(team) == pf:
                        return w, team in eliminated, True
                for team in eliminated:
                    if _normalize(team) == pf:
                        return 0, True, True

        # Step 2: word-subset + Jaccard + qualifier guard across all forms
        for part in parts:
            for pf in _pick_forms(part):
                pw = set(pf.split())
                candidates = []
                ...
                if candidates:
                    # Tiebreaker: prefer ESPN team whose raw name has the same
                    # location code as the pick part (e.g. "oh" in "Miami (oh)"
                    # matches "Miami (OH) RedHawks" over "Miami Hurricanes")
                    part_loc = _loc_code(part)
                    candidates.sort(key=lambda x: (-x[0], 0 if _loc_code(x[1]) == part_loc else 1))
                    _, best_team, best_w = candidates[0]
                    best_norm = _normalize(best_team)
                    is_elim = any(_normalize(t) == best_norm for t in eliminated)
                    return best_w, is_elim, True
                for team in eliminated:
                    tw = set(_normalize(team).split())
                    if pw and pw.issubset(tw):
                        if (tw - pw) & _QUALIFIERS:
    ...
    # Build candidate forms for this pick (bidirectional alias support)
    pick_forms = _pick_forms(pick_str)

    # 1 — Exact normalized match (try all forms)
    for pf in pick_forms:
        for team, w in wins.items():
            if _normalize(team) == pf:
                return w, team in eliminated, True

    # 2 — Word-subset fallback with best-match selection + qualifier guard
    #     Try each candidate form; return on the first form that yields any match.
    #     Qualifier guard rejects teams where the ESPN name contains an extra
    #     word marking a *different* institution (state/tech/geo modifier).
    ...
    for pf in pick_forms:
        ...
        if candidates:
            # Tiebreaker: prefer ESPN team whose raw name has the same
            # location code as the pick (e.g. "oh" for "Miami (oh)")
            pick_loc = _loc_code(pick_str)
            candidates.sort(key=lambda x: (-x[0], 0 if _loc_code(x[1]) == pick_loc else 1))
            _, best_team, best_w = candidates[0]
            best_norm = _normalize(best_team)
            is_elim = any(_normalize(t) == best_norm for t in eliminated)
            return best_w, is_elim, True
