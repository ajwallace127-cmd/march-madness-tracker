...
TEAM_ALIASES = {
    ...
    # "byu" intentionally NOT aliased — ESPN displayName is "BYU Cougars",
    # so "byu" word-subset-matches that naturally without any alias.
    "unc":          "north carolina",
    "pitt":         "pittsburgh",
...
def match_team_wins(pick: str, wins: dict, eliminated: set) -> tuple[int, bool, bool]:
    ...
    if "/" in pick_str:
        ...
        # Jaccard scoring ensures "Texas" → "Texas Longhorns" (0.50) beats
        # "Texas Tech Red Raiders" (0.25), so the right team is chosen.
        _GEO_PI = frozenset({"north","south","east","west","central","upper","lower",
    ...
    # Reject matches where the team name has geographic qualifiers not in the pick
    # e.g. "florida" must NOT match "south florida" or "central florida"
    _GEO = frozenset({"north","south","east","west","central","upper","lower",
                      "northern","southern","eastern","western","new","old","mid"})
    for team in eliminated:
        tw = set(_normalize(team).split())
        if pw and pw.issubset(tw):
            if (tw - pw) & _GEO:
                continue   # geographic qualifier present — too ambiguous, skip
            return 0, True, True
