...
def generate_html(standings: list[dict], wins: dict, eliminated: set,
    ...
    for s in standings:
        ...
        for seed in range(1, 17):
            ...
            if p.get("eliminated"):
                ...
                css = "rp-alive"
            elif p.get("has_played"):
                css = "rp-out"   # played but no wins = eliminated
            else:
                css = "rp-wait"
            pts_label = f'<span class="rp-pts">+{p["points"]}</span>' if p.get("points") else ""
            cells += f'<td class="{css}" title="#{seed} seed · {pts_per_win(seed)} pts/win">{team}{pts_label}</td>'
        picks_grid_rows += f"""
        <tr class="rp-row" data-name="{s['name'].lower()}">
          <td class="rp-name-cell">
            <span class="rp-rank">{s['rank']}</span>
            <span class="rp-pname">{s['name']}</span>
    ...
    # Seed header cells
    seed_headers = "".join(
        f'<th class="text-center rp-seed-hdr" title="{pts_per_win(s)} pts/win">#{s}<br><span class="rp-ppw">{pts_per_win(s)}p</span></th>'
        for s in range(1, 17)
    )
...
<head>
  ...
  <style>
    ...
    *, body {{ font-family: 'Inter', sans-serif; color: var(--text); }}
    ...
    td.rp-wait  {{ background: var(--card); color: var(--muted); text-align: center; }}
    ...
    span.rp-wait-leg  {{ background: #F1F5F9; color: var(--muted); }}
    ...
    <div class="tab-pane fade" id="tab-raw" role="tabpanel">
      ...
      <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
        <p class="text-muted small mb-0">
          ...
          <span class="rp-legend rp-alive-leg">● Alive</span>
          <span class="rp-legend rp-out-leg">✗ Out</span>
          <span class="rp-legend rp-wait-leg">◌ Waiting</span>
        </p>
      ...
      <div class="rp-wrap table-responsive">
        <table class="table mb-0 table-sm" id="rp-table">
          <thead>
            <tr>
              <th class="rp-name-hdr">Participant</th>
              {seed_headers}
            </tr>
          </thead>
          <tbody id="rp-body">
